...
import { 
  ...
  Globe, Zap, LayoutGrid, Terminal, Cpu, ArrowRight 
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <header className="border-b border-white/5 p-6 sticky top-0 z-50 glass-panel">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded bg-gradient-to-br from-green-400 to-blue-500 flex items-center justify-center font-bold text-black text-xl">
              P
            </div>
            <div>
              <h1 className="text-2xl font-bold tracking-tight text-white flex items-center gap-2">
                PRISM ID <span className="text-xs px-2 py-0.5 rounded-full bg-white/10 text-green-400 font-mono">v2.0</span>
              </h1>
              <p className="text-xs text-slate-400 uppercase tracking-widest">Universal • Sortable • Sexy</p>
            </div>
          </div>
          
          <nav className="flex gap-1 bg-white/5 p-1 rounded-lg">
            {['generate', 'compare', 'spec'].map(tab => (
              <button
                key={tab}
      ...
      <footer className="border-t border-white/5 py-8 mt-12 text-center text-slate-500 text-sm">
        <p>Prism ID Specification v2.0 • Designed for the modern web</p>
      </footer>
    </div>
  );
}
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="grid grid-cols-2 gap-2">
              {Object.keys(PrismID.CONFIG).map(type => (
                <button
                  ...
                  className={`px-3 py-2 rounded-lg text-sm font-mono border text-left transition-all ${
                    config.type === type 
                      ? 'border-green-500/50 bg-green-500/10 text-green-400' 
                      : 'border-white/5 bg-white/5 text-slate-400 hover:border-white/10'
                  }`}
                >
                  <div className="font-bold">{PrismID.CONFIG[type].name}</div>
                  <div className="text-xs opacity-60">{PrismID.CONFIG[type].bits} bits</div>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="relative">
              <input 
                ...
                className="w-full bg-black/30 border border-white/10 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-green-500/50 font-mono"
        ...
        <div className="glass-panel p-6 rounded-xl">
          ...
          <div className="space-y-4 text-sm">
            <div className="flex justify-between items-center">
              ...
              <span className="font-mono text-blue-400">{PrismID.CONFIG[config.type].layout.time > 0 ? 'Yes (Ordered)' : 'No'}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-slate-500">Entropy</span>
              <span className="font-mono text-purple-400">{PrismID.CONFIG[config.type].bits - PrismID.CONFIG[config.type].layout.time * 5} bits</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-slate-500">Checksum</span>
              <span className="font-mono text-green-400">Luhn-31</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-slate-500">Safe Chars</span>
              <span className="font-mono text-orange-400">Base31</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="lg:col-span-2 space-y-4">
          ...
          <div key={idx} className="group relative">
            ...
            <div className="relative bg-black/40 border border-white/10 rounded-xl p-6 flex items-center justify-between hover:border-white/20 transition-all">
              <div className="flex-1 min-w-0">
                <div className="font-mono text-2xl md:text-3xl text-white tracking-wide overflow-hidden text-ellipsis">
                  {id.split('-').map((part, i) => (
                    <span key={i} className={
                      i === 0 && config.prefix ? "text-slate-500" :
                      i === (id.split('-').length - 1) ? "text-green-400" : "text-white"
                    }>
                      {part}{i < id.split('-').length - 1 && <span className="text-slate-700">-</span>}
                    </span>
                  ))}
                </div>
              ...
              <button 
                ...
                className="ml-4 p-3 rounded-lg bg-white/5 hover:bg-white/10 text-slate-300 transition-colors"
        ...
        <div className="mt-8 p-6 bg-blue-500/5 border border-blue-500/10 rounded-xl">
          ...
          <div className="bg-black/50 rounded-lg p-4 font-mono text-sm text-slate-300 overflow-x-auto">
    ...
    <div className="space-y-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          <div key={id.name} className={`glass-panel p-6 rounded-xl border ${id.border} relative overflow-hidden`}>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-white/5 border-b border-white/10">
              ...
              <th className="p-4 text-green-400 font-bold bg-green-500/10">PRISM ID</th>
          ...
          <tbody className="divide-y divide-white/5">
            {features.map((f, i) => (
              <tr key={f.name} className="hover:bg-white/5">
                <td className="p-4 text-slate-300">{f.name}</td>
...
function Specification() {
  return (
    <div className="max-w-4xl mx-auto space-y-8 text-slate-300 leading-relaxed">
      <div className="border-l-4 border-green-500 pl-6 py-2">
        <h2 className="text-3xl font-bold text-white mb-2">The Spec</h2>
        <p className="text-lg text-green-400">Format: [PREFIX]-[TIME]-[ENTROPY]-[CHECK]</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="glass-panel p-6 rounded-xl">
          ...
          <div className="font-mono bg-black/50 p-4 rounded-lg text-green-400 break-all">
        ...
        <div className="glass-panel p-6 rounded-xl">
          ...
          <ul className="space-y-3 text-sm">
            <li className="flex justify-between border-b border-white/5 pb-2">
              <span>Time (48 bits)</span>
              <span className="text-slate-500">Sortable ms precision</span>
            </li>
            <li className="flex justify-between border-b border-white/5 pb-2">
              <span>Entropy (40-74 bits)</span>
              <span className="text-slate-500">CSPRNG randomness</span>
            </li>
            <li className="flex justify-between">
              <span>Checksum (5-8 bits)</span>
      ...
      <div className="space-y-4">
        ...
        <p>
          PRISM ID uses a custom Base31 encoding that is lexicographically sortable. 
          ...
          automatically index in chronological order.
        </p>
      </div>
    </div>
  );
